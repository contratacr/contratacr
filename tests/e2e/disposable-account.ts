...
import { regressionAdminClient } from "./seed";
...
export async function createDisposableAccount(options: {
  ...
  try {
    ...
    const { data: reference, error: referenceError } = await admin
      ...
      .limit(1)
      .single();
    if (referenceError || !reference) throw referenceError ?? new Error("No professional reference fixture found");
    ...
    const { data: professional, error: professionalError } = await admin
      ...
      .insert({
        ...
        is_available: true,
        is_verified: false,
        business_name: `Disposable Regression ${token}`,
        ...
        portfolio_urls: reusableImage ? [reusableImage] : [],
        portfolio_items: reusableImage ? [{
    ...
    return { id, email, password, professionalId };
  } catch (error) {
    ...
    if (deleteProfileError || deleteAuthError) {
      throw new AggregateError(
        [error, deleteProfileError, deleteAuthError].filter(Boolean),
...
export async function cleanupDisposableAccount(account: DisposableAccount | undefined) {
  ...
  if (auditError) failures.push(auditError);
