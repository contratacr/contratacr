...
**Required vs OPTIONAL in the completion widget (sprint 385).** The **% / required checklist = essentials only** (foto · descripción · servicios · ubicación · WhatsApp) so activation stays low-friction — never turn optional sections into required-looking steps. **Identity verification** shows as a dismissible **"Opcional"** row (during or after activation; never counts toward the %). The two **OPTIONAL profile ENHANCEMENTS — Disponibilidad and Casos de éxito — are surfaced only AFTER the profile is complete** (post-activation), as the same dismissible "Opcional" rows (hollow bullet · "Opcional" tag · value-framed benefit · action CTA · explicit "✕ Ahora no"). They **auto-resolve** from the pro record (Disponibilidad: `availability_public===false` (WhatsApp-only by choice) OR a non-empty `availability` weekly schedule; Casos de éxito: any `portfolio_urls`/`portfolio_items`) so they never nag once used/dismissed. Dismiss state persists per-pro (`contratacr_{verify|availability|portfolio}_dismissed_{proId}`). The complete-state subtitle anticipates them ("Tu perfil ya está activo. Mejóralo con estos pasos opcionales para atraer más clientes."). Pattern: **essentials frictionless to activate; optional extras discoverable + invitingly framed afterward, never forced.**
