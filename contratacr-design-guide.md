...
- **PIN interaction — clean Airbnb/Google-Maps model** (rewritten Sprint 165; the canonical spec — don't patch around it):
  ...
  - **The popup "Verificado" matches the /buscar CARD badge EXACTLY**: `Badge variant="verified"` / `.ccr-ver` — solid brand-blue `#009FD9` rounded-full pill, white, `10px`/`600`, padding `2px 8px`. Never green text + `✓`.
...
- **"Otro / no está mi servicio" = free-text service (Sprint 213).** The "Agregar profesión" picker has, at the bottom (separated, shown only when `otro` isn't already a profession), an **"Otro / no está mi servicio"** option — the SAME idea as publicar-proyecto's "otro". It adds the **`otro`** profession ("Otro servicio"/"Other service"; safe to persist — `otro` is a seeded category and the `category_id` FK was dropped in mig 013) and immediately opens the add-service modal so the pro types the custom service as **free text** (e.g. "Vendedor de botellas") — a normal service NAME under `otro` (example placeholder + a help line shown only when `formCategory === "otro"`). **No new column, no schema change.** It's **searchable for free**: the /buscar text filter already matches **`services::text.ilike.%q%`**, so the custom name is found by normal substring search (a client searching "botellas" surfaces the pro). This is plain search discoverability ONLY — NOT any automatic project↔professional matching of "otros" (explicitly out of scope; see the "surfacing, not auto-routing" rule above).
...
- **Cross-location OVERLAP guard = block real overlaps, ALLOW consecutive (Sprint 212 + refined Sprint 216).** A save is validated (`findOverlapConflict`) before writing: a proposed franja must not overlap any OTHER location on the same day. Overlap is **half-open** (`rangesOverlap` = `aS < bE && bS < aE`), so **touching/back-to-back ranges are ALLOWED** — Loc A 08:00–15:00 then Loc B **15:00–17:00** is fine; only a true intersection (A 08:00–15:00 + B 14:00–16:00 → 14:00–15:00) blocks. **No minimum gap / travel time** is ever enforced — adjacent is the pro's call. The block message communicates the slot is **OCCUPIED/taken** (never "disponible", which wrongly reads as free) and names the range + place + a concrete next start, with **no quotes** around the location name (Sprint 217): ES "El horario de {start} a {end} ya está **ocupado** en {place} y no puedes estar en dos lugares al mismo tiempo. Puedes agregar a partir de las {end}; un horario seguido sí se permite." / EN "The {start}–{end} time is already **taken** at {place}…". **Visual guidance** (so the pro picks a free slot, never guesses): each weekday row and the "Cambiar un día" modal show a muted, lock-icon line **"Ocupado en {place}: 8:00 AM–3:00 PM"** / EN **"Taken at {place}: …"** (`occupiedElsewhere`, other locations' merged ranges via `otherOccupiedForWeekday`/`otherOccupiedForDate` + `fmtRanges`/`mergeRanges`, 12-h `to12h`). **No « » angle quotes** around location names in these messages. **Toggling a day ON auto-picks a FREE block** (`suggestFreeFranja`: first gap from 08:00 to the next occupied start, else 08:00–17:00) so a second location never lands on the default 08:00–17:00 that would collide + block + leave the day un-editable. Responsive: guidance is `text-[11px]`, wraps cleanly at ~360px.
