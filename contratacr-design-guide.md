...
> **"Lead card" layout for Solicitudes recibidas + Oportunidades (sprint 421).** Both use the same modern card: `rounded-2xl` chrome → a **person/title header** (avatar + bold name/title + a chip on the right: status for solicitudes, categoría for oportunidades) → a **tinted "highlight" block** (`rounded-xl border border-[#eef0f2] bg-[#f9fafb] px-3.5 py-3`, an uppercase eyebrow + a bold value) holding the ONE key fact = the scan anchor (**the appointment date** for solicitudes, **the presupuesto** for oportunidades, with secondary `servicio·zona` / `zona·plazo` beneath) → secondary text (Nota/description) → an actions footer. Para-otra-persona shows a brand-tint "La cita es para" patient callout. Reuse this lead-card pattern for any "incoming item the pro acts on" list.
