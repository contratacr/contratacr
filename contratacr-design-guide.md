...
The home models **Thumbtack/Bark** (structure/patterns only, ContrataCR brand/content). Keep it focused — **less is more**. In order: **(1) Hero** — a **rotating-word headline** ("`<servicio>`, sin complicaciones." where the leading service word fades/slides through a short list) + the **primary search (service + location)** front and center + "Popular:" quick links; subtitle "Describe tu proyecto y te conectamos con el profesional perfecto."; then a **single fixed hero image** in a half-circle/dome (`borderRadius: 50% 50% 0 0 / 100% 100% 0 0`). **(2) Categorías** (Profesionales para cada proyecto — two-row carousel). **(3) Cómo funciona** (Contrata en tres pasos). **(4) Trust benefits** (Contrata con tranquilidad). **(5) Zonas** (Encuentra profesionales en tu zona — find-by-zone panel). **(6) CTA profesional** (Haz crecer tu negocio con ContrataCR). Then footer. Each section opens with the same **eyebrow chip** (brand-tint pill + dot + UPPERCASE label, e.g. `ASÍ DE FÁCIL` / `CONFIANZA` / `COBERTURA LOCAL` / `PARA PROFESIONALES`) above the bold `#1a2744` heading + one-line gray subtitle.
  - **The four home content sections must each have a DISTINCT treatment** (don't repeat one card-row pattern). Current distinct designs:
    - **Cómo funciona (Contrata en tres pasos) — a typographic JOURNEY:** light `bg-white` section, asymmetric header (heading left / subtitle right). 3 steps on a faint horizontal gradient **path line**, each led by an **oversized ghost numeral** (`text-[7rem] font-black` at `text-[#1a2744]/[0.04]` behind the content) + a **gradient icon node** (`h-14 w-14 rounded-2xl from-[#009FD9] to-[#0078a8]`, `group-hover:-translate-y-1`), then `NN` accent number + title + line. **Staggered reveal** (`FadeInUp delay={i*130}`). Honest "no intermediarios / Tú decides" framing.
    - **Trust benefits (Contrata con tranquilidad) — asymmetric BENTO grid:** on `bg-[#f4f7fa]`. A **large featured pillar** (Identidad verificada) as a **dark gradient `from-[#16a34a] to-[#0f7a37]` card spanning 2 cols** with a white glass icon tile; two small white `rounded-3xl` pillars (Reseñas reales amber, WhatsApp green) with gradient icon tiles + radial glow; and a **wide horizontal support card** (Soporte, brand gradient icon) spanning 2 cols to balance. Hover lift + icon scale, **staggered `FadeInUp`**. Benefits, not features; no invented numbers.
    - **Find-by-zone (Encuentra profesionales en tu zona):** the dark two-pane interactive panel (see the find-by-zone entry above) — already distinct; the **quality bar** for the others.
  ...
  - **Phone-mockup pattern (`app-phone.tsx`):** a clean **fixed** (no animation) iPhone-style frame — gradient bezel, dynamic-island, `rounded-[34px]` screen, deep elevation shadow — showing a **REAL ContrataCR screen**. The in-phone image is **swappable via one constant** `PHONE_SCREEN = { src, alt }`: set `src` to a **self-hosted Cloudinary** screenshot of the actual app (e.g. /buscar) and it renders inside; until then a **faithful on-brand JSX placeholder of our real /buscar** (app bar + service/location search + result cards with avatar, name + green `ShieldCheck` "Verificado", soft profession chip, star rating, "Desde ₡…", "Solicitar servicio" + WhatsApp). **Never fake a generic/unrelated screen** — the mockup only adds value showing the real UI; illustrative sample data only, no platform-wide stat claims.
  ...
  - **Rotating-word animation = VERTICAL SLIDE-UP (masked roll)** (`RotatingLine` in `landing-hero.tsx`): slot-machine style — the **whole word** (not letter by letter) slides up into place from below, **holds** fully visible (~2.2s cycle), then slides up and out as the next word rolls in from below; continuous vertical roll. Implementation: a **clipping mask** (`overflow-hidden`, height `1.18em` ≈ one line, room for accents like í/J) over a vertically-stacked column of all words; the column is moved with `transform: translateY(-index·1.18em)` and a gentle springy ease (`cubic-bezier(0.16, 1, 0.3, 1)`, ~620ms). **Seamless loop:** a cloned first word at the end, then a **no-transition snap back to index 0** (`requestAnimationFrame` x2). **NO typewriter, NO caret/pipe.** Each word is `display:flex; justify-center` so it stays **horizontally CENTERED** above the static `sin complicaciones.` line (the headline lines center together; word-length changes cause no layout shift). **Respects `prefers-reduced-motion`** → a static word, no roll. Pacing comfortable/elegant.
