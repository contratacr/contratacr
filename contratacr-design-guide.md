...
> **Status filter tabs (sprint 419, supersedes 413/408).** `StatusFilterTabs` (shared by Solicitudes recibidas, Mis solicitudes, Solicitudes publicadas, Oportunidades) is a **clean wrapping row of pills** — `flex flex-wrap gap-2`, active pill = brand-blue fill, inactive = white/border. **NO per-filter count badges** (removed sprint 419): modern booking/marketplace apps use plain filter chips and reserve count badges for real notifications (the bell); a count on every filter is clutter. **Count-badge colour rule:** every count badge in the app (notification bell, account-menu/drawer notifications, panel sidebar, mode switch, "Más" sheet) is **solid brand-blue `#009FD9` with white text** — NEVER red (red clashes with the navy/blue palette); identical on desktop and mobile.
