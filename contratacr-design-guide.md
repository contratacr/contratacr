...
- **Click a pin → popup mini-card**: a second `AdvancedMarkerElement` floated above the pin — a white **`rounded-[14px]`** card (~**240px**, soft shadow, Inter) with an **initials avatar (light-blue `#EBF5FB`/`#009FD9`)**, name (`#111827`) + green **"✓ Verificado"** (`#16a34a`/EN "Verified") when verified, profession labels, ★ rating (N), and the **"₡… /unit" price in blue `#008ce0`**; the whole card links to the profile, with a close ×. Closes on map click / another pin / ×. **Desktop hover** also opens this card as a preview (150ms close-debounce so pin→card / pin→pin doesn't flicker); mobile = tap.
