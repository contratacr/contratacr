...
- **`ContrataCRLogo`** (in `landing-navbar.tsx`) = **mark + wordmark**; pass **`chip`** on dark backgrounds (footer) to seat the mark in a small white rounded square for contrast. **`ContrataCRMark`** = the icon alone (used in the **compact/scrolled header**, pinned left so the search stays centered). Both the app-wide `LandingNavbar` and the legacy `layout/navbar` render `ContrataCRLogo`, so login / 404 / dashboards inherit it.
