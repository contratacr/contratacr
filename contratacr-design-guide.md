...
- **Hide, don't just disable:** on a pro's own /buscar card and public profile, hide the WhatsApp/call/Solicitar actions and show a clear **"Este es tu perfil"** state (link to their dashboard). Exclude a pro's own projects from the "propose" feed. Always keep the **server guard** too (booking + proposals APIs reject self-actions) — UI hiding is not enough.
...
**Uniform time chips (everywhere times are listed as chips).** Time chips must be a **uniform width in an even grid**, not text-sized in a `flex-wrap` (12h text like "8:00 AM" vs "12:00 PM" makes ragged rows). Use a **`grid` with fixed columns** (e.g. `grid-cols-3 sm:grid-cols-4` in the availability list; `grid-cols-4 sm:grid-cols-5` in the booking picker), each chip `w-full` + **`tabular-nums`** + `text-center` (or `justify-between` when the chip has a remove-X, with the time `truncate` and the X `shrink-0`). Same treatment in the availability "Tus horarios próximos" and the booking time picker.
