...
### Locations step — "¿Cómo ofreces tus servicios?" (registration AND profile editor)
The two work modes are **two separate, clearly-delimited panels**, never one mixed block. Same pattern in registration and the panel editor (keep them identical).
- **Two toggles** (`Trabajo desde un lugar fijo`, `Me desplazo donde el cliente`) — both can be on. **Fixed is listed first**, to match the panel order below.
- **PANEL 1 — Fixed location** (`WorkplacesPicker`): order is **provincia → cantón FIRST** (the authoritative areas that drive `/buscar` filtering; cantón disabled until provincia is set), an optional place name, and THEN an **optional** map pin (search / click / "use my location") for visual precision. **The pin is coordinates only — never re-ask or auto-overwrite the provincia/cantón already chosen** (no duplicate questions). "Agregar lugar" is disabled until provincia+cantón are set; the pin stays optional. Multiple fixed places supported.
- **PANEL 2 — Travel/coverage** (`CoverageAreaSelector`): a SEPARATE panel for the hierarchical zones (un cantón / toda una provincia / todo el país), mixable. Never interleaved with the fixed-location map.
...
- **Placement:** the `ProfileCompletion` card sits at the **TOP of the dashboard**, full-width **above** the sidebar+content columns (a prominent banner), **not nested inside the "Mi perfil" section**. Shown on the profile tab; it returns `null` once the profile is 100% AND identity is verified (keeps the panel clean).
- **Design:** a clean **linear progress bar** (not a ring) + %, then benefit-framed **pending rows** (flat — no per-item border boxes, hover tint only) that deep-link to the right tab, a compact **done-items** chip row under a light divider, and identity verification as a **separate tinted CTA row** (not counted in the %).
...
- **Default workday 8:00 a.m. – 5:00 p.m.** The slot generator pre-fills `Desde 08:00 → Hasta 17:00` (for today, the start floor bumps to the next full hour so a past time is never offered). The pro adjusts freely; picking "Desde" auto-bumps "Hasta" to stay valid.
- **Clean, low-border layout** (consistent with the dashboard de-nesting): the controls live in **soft tinted panels (`bg-[#f9fafb]`, no hard border)** with plain headings — **no numbered "STEP 1/STEP 2" badges or bordered step boxes**. Two blocks: "Cómo te contactan los clientes" (the privada / llamada / videoconsulta toggles, divider-separated) and "Agrega tus horarios" (the generator). Upcoming slots render per-day, grouped by Mañana/Tarde/Noche, as uniform same-width time chips.
