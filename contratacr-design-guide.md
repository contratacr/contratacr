...
> **Status filter tabs — mobile pattern (sprint 408).** `StatusFilterTabs` (the shared Confirmadas/Finalizadas/Canceladas + proyectos' 4-bucket pills) is a **horizontal-scroll row on mobile** (`flex gap-2 overflow-x-auto hide-scrollbar`, each pill `shrink-0 whitespace-nowrap`) so the long Spanish labels never wrap or overflow at ~360px, and a **normal wrapping row on desktop** (`sm:flex-wrap sm:overflow-x-visible`). Use this same pattern for any horizontal filter/tab row that risks overflow on small screens (the `.hide-scrollbar` util is in `globals.css`).
