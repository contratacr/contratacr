...
- **Profile contact card reuses ProfessionalSchedule in `stacked` mode (Sprint 179).** The
  ...
  slot). In stacked mode the contact buttons (WhatsApp green + **Llamar outlined**) ALWAYS show, plus a
  filled "Ver horario completo" when bookable — i.e. bookable → **Ver horario completo + WhatsApp +
  Llamar**; not bookable → WhatsApp + Llamar (coral note above). **NO "Solicitar servicio" on the
  profile** (the WhatsApp/Llamar buttons replace it; Sprint 181). The /buscar card (non-stacked) is
  unchanged (bookable → only "Ver horario completo"; Llamar filled). The profile fetches
  `availability_slots` itself (`profileSlots`). Verificación (Sobre mí) + the header use the canonical
  blue `Badge variant="verified"` pill — never green text.
...
- **Four stacked low-border cards, NO step badges:** (1) "Disponibilidad privada" toggle + subtitle; (2) **"HORARIO PARA"** — location **tabs** (workplaces + **"A domicilio"** when the pro travels; **"+ Agregar lugar"** → `?tab=profile`) + a **Profesión** selector when multi-profession — *each (location, profession) pair has its own weekly schedule*; (3) **"Mis horarios de siempre"** — a **"Cada cita dura"** duration selector (30/45/60/90/120 min) + **7 Monday-first weekday rows**, each a toggle + one-or-more time **franjas** (`TimeSelect` Desde–Hasta + remove) + **"Agregar franja"** + **"Igual a todos los días"** (copies that day's franjas to all 7); (4) **"¿Un día distinto?"** — date exceptions via the **"Cambiar un día"** modal (a month **calendar** + three modes: **"Agregar horas extra"** / **"Horario distinto ese día"** / **"Cerrar ese día"**). Each saved exception lists as a **date-pill row** (month + day) with Editar/Eliminar.
- **A pro can't be in two places at once:** the materializer dedupes by **(date, time)** across ALL locations/professions (`availability_slots` is UNIQUE per `professional+date+time`; first writer wins). Past/too-soon times are filtered (CR, 15-min lead); a configured franja always yields **≥1** bookable time. Legacy pros with manually-created slots and **no** template keep those slots until they adopt the weekly editor (the mount-regenerate is **skipped when there is no template**, so nothing is wiped pre-adoption).
- **"A domicilio" + multi-location/profession** are unchanged downstream: materialized slots carry `location_id`/`category_id`, and `cov_domicilio` renders as the **"A domicilio"** tab on /buscar (no street address). `A_DOMICILIO_LOC = "cov_domicilio"` (starts with `cov_` so it flows through the coverage-zone path). The editor's empty-state `needLocation` copy points pros to add a workplace or enable "Me desplazo donde el cliente".
