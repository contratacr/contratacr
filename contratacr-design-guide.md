...
> **PENDING DECISION — notifications model + emails (sprint 403, recommendation only).** Recommended in-app: **per-mode inside the panel** (the Notificaciones tab filters to the active mode via `notificationContext`) + the **navbar bell as the cross-mode overview** (total-unread counter; replace the ambiguous single "Ver todas" with two tappable rows **"Como cliente (N)" / "Como profesional (N)"** → `?tab=notifications&mode=use|offer`) + a **dot on the inactive mode** in the panel switch when it has unread — so the user is aware of the other mode WITHOUT switching. Recommended emails: ADD **propuesta → client** (low 1:1 volume); **hold/digest the oportunidad email** (one project → N pros = the volume risk; in-app/WhatsApp now, daily digest later); keep solicitud→pro (already on); lean on WhatsApp Cloud API (bypasses the Brevo ~300/day free cap). Awaiting owner go-ahead before implementing.
