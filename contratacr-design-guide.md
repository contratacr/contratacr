...
### Autocomplete dropdowns: ONE shared, keyboard-aware primitive (Sprint 228)
ALL typing-autocomplete suggestion lists use **`components/ui/anchored-dropdown.tsx`** — `useAnchoredPosition(anchorRef, open, maxHeight)` + `<AnchoredDropdown anchorRef open maxHeight className>`. It encapsulates the portal/`fixed`/flip pattern above AND is **mobile-keyboard-aware**: it measures available space with **`window.visualViewport`** (height + `offsetTop`), NOT `window.innerHeight` — because on iOS `innerHeight` doesn't shrink for the on-screen keyboard, so the old math opened the list DOWN into the area behind the keyboard, reading as "the dropdown covers the field". It opens **below by default, flips above only when the *visible* space below is cramped**, clamps `max-height` to the visible space (internal scroll, `overscroll-contain`), and recomputes on scroll/resize + `visualViewport` resize/scroll (keyboard show/hide). **The field is never covered.** Anchor to the field's wrapper (its rect drives width + position). Used by: HeroSearch (service+location), `/buscar` sidebar search + MobileServiceSearch, CategorySearch, LanguagesInput, AseguradorasInput, AdminUserSearch, landing-navbar (`CategoryAutocomplete` + compact bars). **When you build a new typing autocomplete, use `<AnchoredDropdown>` — never a bare `absolute top-full` list.** Google Places renders its own `.pac-container` (positioned by Google below the input); we only cap it via CSS (`max-height: 45vh; overflow-y:auto`) so a long list scrolls instead of overflowing.
