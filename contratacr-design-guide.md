...
>
> **The account model is ONE account + two capabilities (Airbnb — sprint 376).** Capabilities: **SEEK** (search/hire) is **always on** for every account; **OFFER** (provide services) is **unlocked by completing the professional profile**. The canonical capability flag is **`profiles.is_provider`** (migration 068): a DB trigger flips it true the moment a `professionals` row is created and false if it's removed (drift-proof), and it's mirrored into `user_metadata.is_provider` for instant client reads (use `canOffer(user)` from `lib/auth/capabilities.ts`; it also honors the legacy `role==="professional"`). The legacy `profiles.role`/`user_metadata.role` is KEPT for back-compat (admin/notifications/RLS) but is **no longer the app's identity switch** — there is no permanent "client vs professional" identity, just an account that may or may not have unlocked offering. There are **no real users yet**, so this was a clean migration, not a workaround. **Registration is just "where you start":** "Busco servicios" → unified panel in Usar mode; "Ofrezco servicios" → complete the professional profile → unlock offering → Ofrecer mode. **Becoming an offerer** works from any account (the "Ofrecer mis servicios" CTA / the offer-mode activation gate → `/registro/profesional`). The **role chooser** is framed as **"where to start," not a permanent side** (sprint 375): heading **"¿Por dónde quieres empezar?"** (en "Where do you want to start?"), two action cards **"Busco servicios"** (→ lightweight client signup) / **"Ofrezco servicios"** (→ full pro signup), then a **prominent styled "you can do both" callout** directly under the cards — a tinted `#EBF5FB` panel with `ring-1 ring-inset ring-[#009FD9]/15` + an `ArrowLeftRight` brand-blue icon, a bold lead **"Puedes hacer ambas cosas."** (`bothTitle`) + reassurance **"Esto es solo por dónde empiezas: agrega la otra opción cuando quieras desde tu panel."** (`bothNote`). **No redundant third "Ambas" button** (it would route to the same flow). Same treatment on `/registro` and `/onboarding` (shared `registerChoice`). You start with ONE action and **add the other later**: add offering = the **"Ofrecer mis servicios"** CTA (→ `/registro/profesional`); add seeking = already present (every pro panel has the "Busco servicios" group). Existing accounts map cleanly (client→seek, professional→offer+seek). **"profesional" is KEPT** (industry-standard, dignifies informal trades): the ACTION labels organize the panel/chooser, but the **person nouns stay** — the offerer sees the seeker as **"cliente"**, the seeker sees the offerer as **"profesional"**. Deferred (per safety): a literal `offers/seeks` flag column (role already encodes it) and a combined "both at once" signup.
