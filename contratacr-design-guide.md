...
- **Click / hover a pin → popup mini-card**: a second `AdvancedMarkerElement` floated above the pin — a white **`rounded-[14px]`** card (~**240px**, soft shadow, Inter) with an **initials avatar (light-blue `#EBF5FB`/`#009FD9`)**, name (`#111827`) + the **"Verificado" pill** when verified, profession labels, ★ rating (N), and the **"₡… /unit" price in blue `#008ce0`**; the whole card links to the profile, with a close ×. Closes on map click / another pin / ×. **Desktop hover** also opens this card as a preview (150ms close-debounce so pin→card / pin→pin doesn't flicker); mobile = tap.
  ...
  - **NO hover vibration — the REAL root cause + fix** (Sprint 155, after 148's dedupe wasn't enough): the popup card is anchored just ABOVE the pin, and the pin's **`is-active` 1.15× scale** grows its top edge up UNDER the card; an interactive card (`pointer-events:auto`) then **STEALS the hover** from the pin → pin `mouseleave` → the pin un-scales → the cursor is over the pin again → `mouseenter` → … a continuous flicker loop **no debounce/dedupe can stop** (the pin genuinely keeps losing/regaining hover). **FIX (geometry-independent): the HOVER preview popup is `pointer-events:none`** (set on `wrap.firstElementChild`), so it can NEVER capture the pointer / steal the pin's hover → the pin stays hovered, the card is rock stable. **Click/TAP popups stay `pointer-events:auto`** (a deliberate gesture; the cursor settles ON the card → no pin↔card alternation) so the card is still clickable. `openPopup(…, interactive)` keys the popup `H:`/`C:` (a click upgrades the hover preview); the `popupKeyRef` dedupe (148) is kept too. Clusters don't scale (never `is-active`) so their popup stays interactive on hover.
