...
### Home / landing page (service-directory model)
The home follows a clean **service-directory marketplace** structure (Bark-style *patterns*, ContrataCR brand/content/categories — never their assets). In order: **(1) Header** — see "Header" below (minimal, with the **Categorías** mega-menu + scroll-to-compact-search). **(2) Hero** — centered: bold headline **"Encuentra al profesional perfecto para ti"** + one-line subtitle **"Recibe cotizaciones gratis en minutos"** + a **service + location search** (a service text field · a **location field with typeahead** — Costa Rica has no postcode, so the user types a provincia/cantón and picks from accent-insensitive suggestions, placeholder "Provincia o cantón" · a "Buscar" button) + a "Popular: …" quick-link row. Keep `#hero-search-sentinel` after the search so the header reveals its compact search on scroll. **(3) Category bands** — **two photo-card rows that auto-scroll slowly in OPPOSITE directions** (top drifts left, next drifts right): continuous, eased/linear, **pause on hover**, disabled under `prefers-reduced-motion`; each row's list is duplicated for a seamless loop. **(4) Grouped sections** — "Hogar y jardín", "Salud y bienestar", "Bodas y eventos", "Servicios para empresas", each a small photo-card grid + a "Ver todas" → /categorias. Then footer.
- **Uniform category card (one design everywhere).** EVERY category card on the home — bands and grids alike — uses the **exact same** markup + hover: an `aspect-[4/3] rounded-xl` photo over a `#EBF5FB` tint, **image zoom on hover** (`group-hover:scale-105 duration-500`), the label **below** the image (`text-sm font-semibold text-[#1a2744]`, turns `#009FD9` on hover), linking to `/buscar?categoria=<id>`. No card may look different from another.
- **NO "Disponible en línea" / online badges anywhere.** ContrataCR does not surface online availability on category cards — never add that badge.
- **"Categorías más populares" is DEFERRED to post-launch.** Do NOT add a most-popular/"trending" section until there is real usage data **and** enough professional coverage — promoting categories with zero/few professionals sends clients to empty results (worse than omitting it). Only surface categories that can actually be fulfilled.
- **Imagery**: Unsplash (hosts whitelisted in `next.config` — `images.unsplash.com`; **validate every photo id** returns 200) or authentically Costa Rican photos. Still **no** phone mockups, "coming soon" app section, or unaffiliated brand-logo strips (false-partnership / veracity risk).
...
**Header** (marketing/public, `LandingNavbar`): **minimal, service-directory style.** Logo (left) · a **single "Categorías" mega-menu** · far right **"Iniciar sesión"** + a prominent pill **"Únete como profesional"**. Do **not** use multiple top-nav category menus — consolidate ALL service navigation into the one Categorías mega-menu, an Explore-style panel: a left column of service **groups** (`CATEGORY_GROUPS`, emoji + label) that set an **active group on hover**, a middle column listing that group's **items** (→ `/buscar?categoria=<id>`), a **"Servicios populares"** column, and a **"Ver todas"** → /categorias. **Scroll behavior:** once the hero scrolls past, the header condenses to a compact **"Busca un servicio…"** search bar (IntersectionObserver on `#hero-search-sentinel`, fallback `scrollY`); back at the top it returns to the minimal state. The **logged-in** state keeps Mi panel + notifications + avatar menu + language toggle; the logged-out header is the minimal version above.

**Footer** (`bg-[#111827]`): brand blurb + socials, then link columns — Para clientes · Para profesionales · Soporte · **ContrataCR** (Sobre nosotros · Para profesionales · Términos · Privacidad) — and a bottom bar (© · Términos · Privacidad · "Hecho en Costa Rica"). **Header/footer "Registra tu perfil" must be session-aware** (→ panel when logged in; `SmartRegisterLink` or the same role check). **Legal/about pages** (`/sobre-nosotros`, `/terminos`, `/privacidad`) use a brand-glow title + `max-w-3xl` prose sections; legal copy is **ContrataCR-specific**, keeps the **intermediary framing** (connects, doesn't guarantee outcomes; no commissions; payments off-platform) and references **Ley 8968**. **404 / error** pages are friendly and on-brand with a clear path to home/search (never a raw failure screen). **Contact/support** (`/soporte`): a ticket form (name optional, email/subject/message required, attachments) that **prefills the logged-in user** and welcomes guests; it creates an **admin ticket** (`support_tickets`, visible in **Admin → Soporte**) and emails support, plus a **WhatsApp** option for urgent issues — set the expectation (reply by email < 24 h).
