...
- **Click / hover a pin → popup mini-card**: a second `AdvancedMarkerElement` floated above the pin — a white **`rounded-[14px]`** card (~**240px**, soft shadow, Inter) with an **initials avatar (light-blue `#EBF5FB`/`#009FD9`)**, name (`#111827`) + the **"Verificado" pill** when verified, profession labels, ★ rating (N), and the **"₡… /unit" price in blue `#008ce0`**; the whole card links to the profile, with a close ×. Closes on map click / another pin / ×. **Desktop hover** also opens this card as a preview (150ms close-debounce so pin→card / pin→pin doesn't flicker); mobile = tap.
  ...
  - **NO hover vibration** (Sprint 148): the popup is deduped via **`popupKeyRef`** — it is rebuilt ONLY when the target pro/cluster key changes. Re-entering the SAME pin re-activates it WITHOUT tearing down + rebuilding the `AdvancedMarkerElement` (and without re-scrolling the card). The old code rebuilt the popup on *every* `mouseenter`, so the pin↔card overlap + the `is-active` re-scale could drive a rapid enter/leave loop that rebuilt the card each mousemove = the "vibration". Keep openPopup/openClusterPopup idempotent for the current key.
