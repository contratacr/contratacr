...
- **All /buscar sidebar/drawer filter fields are ONE uniform size.** The unified service/search input + Provincia + Cantón + Ordenar por + Aseguradora are each **`h-10 w-full rounded-xl`** (the search input is a raw field matching the shared `SelectTrigger` height; the four dropdowns inherit `SelectTrigger`'s `h-10 w-full`). No field may carry an inline side control (e.g. a clear X) that changes its width — that breaks the uniform alignment. (The mobile `chips` variant is a separate horizontally-scrollable pill row and is currently unused on /buscar.)
