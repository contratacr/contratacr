...
The home models **Thumbtack/Bark** (structure/patterns only, ContrataCR brand/content). Keep it focused — **less is more**. In order: **(1) Hero** — a **rotating-word headline** ("`<servicio>`, sin complicaciones." where the leading service word fades/slides through a short list) + the **primary search (service + location)** front and center + "Popular:" quick links; subtitle "Describe tu proyecto y te conectamos con el profesional perfecto."; then a **single fixed hero image** in a half-circle/dome (`borderRadius: 50% 50% 0 0 / 100% 100% 0 0`). **(2) Categorías** (Profesionales para cada proyecto — two-row carousel). **(3) Cómo funciona** (Contrata en tres pasos). **(4) Trust benefits** (Contrata con tranquilidad). **(5) Zonas** (Encuentra profesionales en tu zona — find-by-zone panel). **(6) CTA profesional** (Haz crecer tu negocio con ContrataCR). Then footer. Each section opens with the same **eyebrow chip** (brand-tint pill + dot + UPPERCASE label, e.g. `ASÍ DE FÁCIL` / `CONFIANZA` / `COBERTURA LOCAL` / `PARA PROFESIONALES`) above the bold `#1a2744` heading + one-line gray subtitle.
  - **Cómo funciona (Contrata en tres pasos) — a connected JOURNEY, not 3 isolated boxes:** on a `bg-gradient-to-b from-white to-[#eef4f9]` section (soft radial brand glow up top for depth). The 3 steps share a **horizontal gradient "path" line** behind them (desktop, `from-[#009FD9]/10 via-/45 to-/10`) with a **forward `ChevronRight`** in the gaps — visually linking 1 → 2 → 3. Each step leads with a **large prominent number** (`5.5rem` white ring disc holding a `from-[#009FD9] to-[#0078a8]` gradient circle) sitting ON the path, then an elevated `rounded-3xl` white card (icon in a brand-tint `rounded-2xl` tile, title, one honest line). Cards **lift on hover** (`hover:-translate-y-1.5` + deeper shadow), and each step **reveals on scroll, staggered** (per-item `FadeInUp delay={i*130}`). Steps: Describe tu proyecto · Compara profesionales · Coordina y contrata. Framing = no intermediarios / "Tú decides con quién trabajar" (never "garantizamos").
  - **Trust benefits (Contrata con tranquilidad) — prominent pillars with depth:** on a `bg-gradient-to-b from-[#eef4f9] to-white` section so the white cards pop. 4 `rounded-3xl` cards, each with a **bold gradient icon tile** (`h-16 w-16 rounded-2xl`, white icon) in the pillar's color + a **matching top accent bar** + a **soft radial glow** behind the icon: Identidad verificada (green `#16a34a→#15803d`) · Reseñas reales (amber `#f59e0b→#d97706`) · Coordina por WhatsApp (WhatsApp green `#25D366→#1da851`, official `WhatsAppIcon`) · Soporte (brand `#009FD9→#0078a8`). **Lift + icon-scale on hover** (`hover:-translate-y-2`, `group-hover:scale-105`), **staggered scroll reveal** (`FadeInUp delay={i*110}`). Benefits, **not** technical features; no invented numbers.
  - **DEPTH/CONTRAST STANDARD (avoid flat white boxes):** every home content section should carry intentional depth — a **layered/gradient section background** (not bare `bg-white`), **elevated cards** (`rounded-3xl`, `shadow-[0_10px_40px_rgba(16,39,68,0.06)]` → deeper on hover), **an accent color with presence** (gradient icon tiles, accent bars, soft radial glows), clear **type hierarchy** (eyebrow chip → bold `#1a2744` heading → gray subtitle), and **subtle staggered reveal-on-scroll + smooth hover lift/scale**. The dark `#1a2744` panels (find-by-zone left rail, grow-business CTA) are the **quality bar** — light sections must feel as deliberate. Keep it elegant and fast (transform/opacity only).
