...
The home models **Thumbtack/Bark** (structure/patterns only, ContrataCR brand/content). Keep it focused — **less is more**. In order: **(1) Hero** — a **rotating-word headline** ("`<servicio>`, sin complicaciones." where the leading service word fades/slides through a short list) + the **primary search (service + location)** front and center + "Popular:" quick links; subtitle "Describe tu proyecto y te conectamos con el profesional perfecto."; then a **single fixed hero image** in a half-circle/dome (`borderRadius: 50% 50% 0 0 / 100% 100% 0 0`). **(2) Categorías** (Profesionales para cada proyecto — two-row carousel). **(3) Cómo funciona** (Contrata en tres pasos). **(4) Trust benefits** (Contrata con tranquilidad). **(5) Zonas** (Encuentra profesionales en tu zona — find-by-zone panel). **(6) CTA profesional** (Haz crecer tu negocio con ContrataCR). Then footer. Each section opens with the same **eyebrow chip** (brand-tint pill + dot + UPPERCASE label, e.g. `ASÍ DE FÁCIL` / `CONFIANZA` / `COBERTURA LOCAL` / `PARA PROFESIONALES`) above the bold `#1a2744` heading + one-line gray subtitle.
  - **Cómo funciona (Contrata en tres pasos):** 3 cards on `bg-[#f4f7fa]` — each a `rounded-2xl` white card with a **brand-tint number chip (top-left) + muted lucide icon (top-right)**, title + one honest line. Steps: Describe tu proyecto · Compara profesionales · Coordina y contrata. Framing = no intermediarios / "Tú decides con quién trabajar" (never "garantizamos").
  - **Trust benefits (Contrata con tranquilidad):** the 4 client pillars in tinted icon tiles — Identidad verificada (green `#dcfce7/#16a34a`) · Reseñas reales (amber `#fef3c7/#d97706`) · Coordina por WhatsApp (green, official `WhatsAppIcon`) · Soporte cuando lo necesites (brand `#EBF5FB/#009FD9`). Benefits, **not** technical features; no invented numbers.
  ...
  - **Category carousel (Bark-style two-row band):** under the "Profesionales para cada proyecto" heading the cards live in a **full-bleed, two-row auto-scrolling marquee**, slow and continuous (CSS `transform` keyframes `cat-scroll-left`/`cat-scroll-right`, ~70s desktop / ~48s mobile, `linear infinite`). **Non-negotiables (the effect is fragile — keep all four):**
    1. **Opposite directions** — **top row drifts LEFT (`0 → -50%`), bottom row drifts RIGHT (`-50% → 0`)**. The clearly-alternating motion is the whole point; never let both rows move the same way.
    2. **Uniform cards** — every card is the **EXACT same fixed width/height** (`w-[230px] sm:w-[284px] h-[160px] sm:h-[196px]`, Bark-like ~3:2) regardless of label length; long labels **clamp to 2 lines** (`line-clamp-2`), they never widen the card. (Fixed width on each card + `shrink-0` — never flex-auto sizing, which is what made cards vary before.)
    3. **No on-screen duplicates (the real root cause)** — a single set must be **WIDER than the viewport**, otherwise the loop's off-screen copy bleeds into view and you see a category twice (this broke on ultra-wide ~2558px monitors when a 9-card row was only ~2300px). Two rules together fix it: (a) the **two rows hold DISTINCT category sets** (no category in both rows); (b) each set is wide enough — with ~284px cards + `mr-7` (28px) spacing, a 9–10 card set is ~2800–3120px, comfortably past an ultra-wide screen. Spacing is a **right-margin on each card (`mr-4 sm:mr-7`), NOT flex `gap`**, so each set is exactly `N×(card+margin)` wide and `translateX(-50%)` lands pixel-perfect (no drift / snap). The doubled set is `aria-hidden`. **If you ever add categories, keep each row's set ≥ the widest target viewport** (more distinct cards per row = safer).
    4. **Identical Bark-style card** — `rounded-2xl`, soft shadow, dark bottom gradient + white label, same `card-lift` hover (gentle 4px lift) + image `scale-[1.07]`; ONE self-hosted Cloudinary image per id; **no "Disponible en línea"/"available online" badge anywhere**.
    Plus: **pauses on hover** (`.cat-carousel:hover .cat-track { animation-play-state: paused }`), **respects `prefers-reduced-motion`** (animation off → normal horizontal scroll), soft **edge `mask-image`** fade at the rails. Each card → `/buscar?categoria=<id>`.
