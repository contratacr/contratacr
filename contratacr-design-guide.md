...
The home models **Thumbtack/Bark** (structure/patterns only, ContrataCR brand/content). Keep it focused — **less is more**. In order: **(1) Hero** — a **rotating-word headline** ("`<servicio>`, sin complicaciones." where the leading service word fades/slides through a short list) + the **primary search (service + location)** front and center + "Popular:" quick links; subtitle "Describe tu proyecto y te conectamos con el profesional perfecto."; then a **single fixed hero image** in a half-circle/dome (`borderRadius: 50% 50% 0 0 / 100% 100% 0 0`). **(2) Categorías** (Profesionales para cada proyecto — two-row carousel). **(3) Cómo funciona** (Contrata en tres pasos). **(4) Trust benefits** (Contrata con tranquilidad). **(5) Zonas** (Encuentra profesionales en tu zona — find-by-zone panel). **(6) CTA profesional** (Haz crecer tu negocio con ContrataCR). Then footer. Each section opens with the same **eyebrow chip** (brand-tint pill + dot + UPPERCASE label, e.g. `ASÍ DE FÁCIL` / `CONFIANZA` / `COBERTURA LOCAL` / `PARA PROFESIONALES`) above the bold `#1a2744` heading + one-line gray subtitle.
  - **The four home content sections must each have a DISTINCT treatment** (don't repeat one card-row pattern). Current distinct designs:
    - **Cómo funciona (Contrata en tres pasos) — a premium connected JOURNEY:** `bg-gradient-to-b from-white … to-[#eef4f9]` + radial brand glow. A **gradient progress path** threads through **glowing gradient icon nodes** (`h-[4.5rem] w-[4.5rem] rounded-2xl from-[#009FD9] to-[#0078a8]`, blurred radial glow behind, `ring-[6px] ring-white`, hover-raise) sitting on the line, with a small **white connector dot + `ArrowRight`** between steps. Below each node an **elevated card** (`rounded-3xl`, deep shadow, hover lift) carries a **ghost numeral** watermark, a `Paso NN` brand pill, title + line. **Staggered reveal** (`FadeInUp delay={i*140}`). Honest "no intermediarios / Tú decides" framing.
    - **Trust benefits (Contrata con tranquilidad) — asymmetric BENTO grid:** on `bg-gradient-to-b from-[#eef4f9] to-white` (two faint radial glows). A **large featured pillar** (Identidad verificada) = dark gradient `from-[#16a34a] to-[#0c6e31]` spanning 2 cols, with a **decorative shield watermark**, white glass icon tile, and two honest detail chips (`Cédula validada`, `Registro oficial`); two small white `rounded-3xl` pillars (Reseñas amber, WhatsApp green) with **glowing** gradient icon tiles; a **wide horizontal support card** (brand gradient icon) spanning 2 cols. Hover lift + icon scale, **staggered `FadeInUp`**. Benefits, not features; no invented numbers (detail chips must stay factual — we validate cédula against the official registry).
    - **Find-by-zone (Encuentra profesionales en tu zona) — premium dark/light two-pane panel:** `rounded-[28px]` with a deep shadow + `ring-1`. Left **dark `#111a2e` rail** with a **radar-rings + `MapPin` watermark + brand glow** motif, the glowing "Usar mi ubicación" button, and the honest disclaimer; right white pane with **gradient-filled active province tab**, a province header (icon tile + green coverage count), a prominent "Ver profesionales" button, and cantón chips each with a **status dot** (hover → brand tint). One of the **quality-bar** sections.
