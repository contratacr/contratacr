...
- **Same CARD container as client signup.** Both the professional and client registration render their form inside an identical clean white card — `bg-white rounded-3xl shadow-sm border border-[#e5e7eb] p-8` on a `bg-[#fafafa]` page, vertically centered (`flex items-center justify-center px-4 py-12`, `w-full max-w-md`). The multi-step pro form (and its OTP screen) live INSIDE that one card (Typeform-style single card with step indicator). Keep the two flows visually consistent — don't let either render its content bare on the page.
