...
Unverified professionals **are listed and bookable** (not hidden) but, when contacting/booking one, show an honest notice: "Este profesional aún no ha verificado su identidad. Coordina con cuidado." Wording **informs, never endorses** — ContrataCR only *displays* status and does not vouch for or guarantee any professional. The pro's panel mirrors this ("Identidad sin verificar · visible para clientes") with the path to verify. **The Verificación tab's SINGLE verify action (sprint 332-333) is the "Verifica tu identidad con tu cédula" card** — enter/confirm an ID number (labelled **"Número de identificación"**, never "Número de cédula (DIMEX/NITE también)") → it's stored + checked against the padrón → a valid national cédula verifies INSTANTLY and overwrites the name from the padrón (profiles + Auth), DIMEX/NITE → manual review. This ONE card covers every non-verified case (correct a cédula that didn't auto-verify, a sin-cédula/revoked pro adding one, a re-check); the old redundant "Verificar mi identidad ahora" re-run button was removed. (Rejected pros also keep the appeal box.) Only **rejected** profiles are hidden from search.
