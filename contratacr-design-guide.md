...
**ONE account, two capabilities, ONE panel with a MODE SWITCH (Airbnb model — sprint 376).** Every account uses the SAME unified panel (physically still at `/dashboard/profesional`; `/dashboard/cliente` is now a thin redirector into it). At the top sits an Airbnb-style **mode switch** — **"Busco servicios" ↔ "Ofrezco servicios"** — that flips the whole panel between the user's two sides. The labels are **first-person on purpose (sprint 381)**: they read as "MY seeking side" / "MY offering side" (it's the user's own info/activity, not an abstract action), they're **identical to the registration chooser** (reinforcing the one-account model), and they avoid forcing "profesional" as an identity. (en "Looking for services" / "Offering services".) Replaces the awkward "Usar servicios" / "Ofrecer servicios".
- **"Busco servicios"** (the SEEK capability — every account has it): Mi perfil (básico) · **Mis solicitudes** · **Mis solicitudes publicadas** · **Mis favoritos**.
- **"Ofrezco servicios"** (the OFFER capability — unlocked by completing the professional profile): Mi perfil (pro) · Servicios · Casos de éxito · Disponibilidad · **Solicitudes recibidas** · **Oportunidades** · Verificación. If the account has NOT unlocked offering yet, this mode shows an **activation gate** ("Empieza a ofrecer tus servicios" → completar el perfil), NOT pro tools.
