...
The home models **Thumbtack/Bark** (structure/patterns only, ContrataCR brand/content). Keep it focused — **less is more**. In order: **(1) Hero** — a **rotating-word headline** ("`<servicio>`, sin complicaciones." where the leading service word fades/slides through a short list) + the **primary search (service + location)** front and center + "Popular:" quick links; subtitle "Describe tu proyecto y te conectamos con el profesional perfecto."; then a **single fixed hero image** in a half-circle/dome (`borderRadius: 50% 50% 0 0 / 100% 100% 0 0`). **(2) Categorías** (Profesionales para cada proyecto — two-row carousel). **(3) Por qué ContrataCR** (`why-contratacr.tsx` — the merged how-it-works + trust **sticky-phone story**). **(4) Zonas** (Encuentra profesionales en tu zona — interactive CR map). **(5) CTA profesional** (Haz crecer tu negocio con ContrataCR — pro-facing, kept separate). Then footer. Each section opens with the same **eyebrow chip** (brand-tint pill + dot + UPPERCASE label, e.g. `ASÍ DE FÁCIL` / `CONFIANZA` / `COBERTURA LOCAL` / `PARA PROFESIONALES`) above the bold `#1a2744` heading + one-line gray subtitle.
  - **The four home content sections must each have a DISTINCT treatment** (don't repeat one card-row pattern). Current distinct designs:
    - **Por qué ContrataCR (`why-contratacr.tsx`) — Thumbtack-style STICKY-PHONE story** (merges the old "Contrata en tres pasos" + "Contrata con tranquilidad" into one client narrative): `bg-white`, two columns. Left = 4 scrolling **points** (Describe tu proyecto · Compara profesionales verificados · Coordina por WhatsApp · Con soporte cuando lo necesites) — each a card; the **active** point (closest to viewport center, deterministic rAF scroll calc — NOT IntersectionObserver) gets `border + shadow + dark icon tile`, the others fade. Right = a **sticky phone** (`PhoneFrame`) that **swaps to the matching real app screen** (`phone-screens.tsx`: SearchScreen / ResultsScreen / ChatScreen / SupportScreen — on-brand recreations of the actual UI, easy to swap for short videos later). On mobile the phone renders **inline** under each point. **Sticky requires NO `overflow-hidden` ancestor** (it silently breaks `position: sticky`). Monochrome icons (serious tone). The standalone tres-pasos / trust-benefits components + the cutout-handshake image are retired by this merge (asset still in Cloudinary if needed).
