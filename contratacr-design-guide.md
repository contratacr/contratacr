...
- **`BookingRequests` card is APPOINTMENT-LED (sprint 341 redesign).** Read order is **WHEN → WHAT → WHO → who-booked**, not a flat list. **Header:** status badge + muted **"Solicitada {date}"** (the REQUEST date — labeled, never bare, so it never reads as the appointment). **PRIMARY block** (the appointment): (1) the **appointment date/time is the prominent lead** — brand `CalendarDays h-5` + **`text-[15px] font-semibold`** value (greyed "Sin fecha definida" when none); (2) **profession · location**; (3) **description**; (4) **who it's for**. For a **third-party** booking, "who" is a tinted **"LA CITA ES PARA"** box (eyebrow brand `#0089bb` + patient name + age pill + `Nac · Cédula/"Sin cédula"`). For a **SELF** booking it collapses to a **single person row** (h-9 avatar + name + age pill + flagged + a compact `tel · cédula · Nac` line) with **no eyebrows** — keep self simple. **Then an aesthetic divider** (`mt-3 pt-3 border-t`) and, **third-party only**, the **"RESERVADO POR"** block (eyebrow `text-[#9ca3af]`, **grey-fallback** h-9 avatar to visually demote it below the patient + requester name + `tel · cédula`). **Contact (WhatsApp) is INLINE next to each person, not in the footer:** the **requester is the PRIMARY contact** (shown wherever the requester appears — the self person row or the "Reservado por" block) because they booked and are the responsible, reachable party; the **patient is a SECONDARY contact shown only when they gave a DISTINCT phone** (`beneficiary_phone !== client_phone`), inside the "La cita es para" box. The footer holds only **status actions** (Confirmar/Cancelar/Marcar completada) + the `self-start` "Reportar" link. **Age markers are ONE consistent pill** for both brackets — `rounded-full bg-[#fef3c7] px-2 py-0.5 text-[10px] font-semibold text-[#92400e]` ("Menor de edad" / "Adulto mayor"). All DOB displays use **`formatDobDMY` (dd/mm/yyyy)**, never raw ISO.
