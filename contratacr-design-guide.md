...
- **Click / hover a pin → popup mini-card**: a second `AdvancedMarkerElement` floated above the pin — a white **`rounded-[14px]`** card (~**240px**, soft shadow, Inter) with an **initials avatar (light-blue `#EBF5FB`/`#009FD9`)**, name (`#111827`) + the **"Verificado" pill** when verified, profession labels, ★ rating (N), and the **"₡… /unit" price in blue `#008ce0`**; the whole card links to the profile, with a close ×. Closes on map click / another pin / ×. **Desktop hover** also opens this card as a preview (150ms close-debounce so pin→card / pin→pin doesn't flicker); mobile = tap.
  - **The popup "Verificado" matches the /buscar CARD badge EXACTLY** (Sprint 148): the same as `Badge variant="verified"` / the card pill — a **solid brand-blue `#009FD9` rounded-full pill, white text, `font-size:10px` / `font-weight:600`, padding `2px 8px`** (= `px-2 py-0.5 text-[10px] font-semibold`) (`.ccr-ver`). **Never** green text + a `✓` check. Keep in sync with the card/profile badge.
  - **NO hover vibration — the ACTUAL root cause + fix** (Sprint 160; the 3rd attempt — 148 dedupe and 155 card-`pointer-events:none` both failed because they targeted the wrong element). The popup is a SEPARATE `AdvancedMarkerElement` anchored at the pin; Google wraps the content in a **`<gmp-advanced-marker>` WRAPPER whose hit-box is at the popup's UN-transformed layout position** — i.e. directly **ON TOP OF the pin** (the card is only shifted up *visually* by the CSS `transform: translateY(-52px)`; transforms don't move the wrapper's layout box). That transparent wrapper, default **`pointer-events:auto`**, **STOLE the pin's hover** → pin `mouseleave` → popup closes → cursor back on pin → `mouseenter` → … the flicker loop. **FIX: `killWrapperPE(wrap)` sets `pointer-events:none` on the Google WRAPPER** (walking up to `<gmp-advanced-marker>`, never the shared pane) so the cursor passes straight THROUGH the popup to the pin — zero steal, rock stable — while the **card/rows keep `pointer-events:auto`** (CSS) and stay clickable (a child's `auto` overrides the wrapper's `none`). Position computed once on open; deduped by `pro.id`. Also dropped scroll-on-hover (it shifted the sticky map → churn). Applied to BOTH the pin and cluster popups.
- **Cluster preview (combined members mini-card)** (Sprint 160): hovering (desktop) / tapping (mobile) a cluster shows a **stable** popup listing the grouped pros (avatar + name + Verificado + ★rating + price, each row → profile; capped at 6 + "+N más"). The same `killWrapperPE` makes it stable — before this, the wrapper-steal made it open-then-instantly-close ("cluster shows nothing"). To separate the pins the user zooms (wheel/pinch) — no zoom button.
