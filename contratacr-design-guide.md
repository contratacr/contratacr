...
### Support tickets — admin-managed, one threaded conversation
ALL support is centralized as **tickets** (no public email/live-chat placeholder; `/contacto` + `/soporte` forms create a ticket). Model: `support_tickets` (status **open · in_progress · resolved · closed**, requester, subject, topic, audit `handled_by_name`/`handled_at`, `last_reply_*`) + a **`support_ticket_messages` thread** (`sender_role` user|admin; distinct name to avoid the legacy `support_messages` table from migration 012). Service-role only (RLS on, no policies).
- **Admin inbox** (`AdminSupport`, Admin → Soporte): list filterable by status (flag "Espera respuesta" when the last message is the user's), open a ticket → **chat thread** (admin right/dark, user left/light), **reply in-panel** (stored in the thread AND emailed to the user), change status with audit. Replying bumps `open → in_progress`.
- **User side** (`SupportTickets`): a **"Soporte" tab in both dashboards** — their tickets + thread + reply box (user msg left/blue here). A user reply re-opens a resolved/closed ticket. "Nueva respuesta" hint when the admin replied.
- **Emails** (`support-notify.ts`, branded, **clickable links only — never raw URLs**): notify the support inbox on a new ticket / user reply ("Abrir en el panel"); notify the user on an admin reply ("Ver conversación"). Honest copy — don't promise instant/24-7.
