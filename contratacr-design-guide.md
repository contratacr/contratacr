...
> **Status filter tabs (sprint 413, supersedes 408).** `StatusFilterTabs` (shared by Solicitudes recibidas, Mis solicitudes, Solicitudes publicadas, Oportunidades) is a **wrapping row of vertical stacks** — each filter = a **count badge ABOVE its pill** + the pill. The count badge is notification-style: **solid brand-blue `#009FD9`/white** when the filter has items, **muted `#f3f4f6`/`#9ca3af`** when 0. The row WRAPS (`flex flex-wrap gap-x-2.5 gap-y-2.5`) so badges are never clipped and nothing overflows at ~360px. Every filter shows its count at a glance.
