...
- **Map:** `mapId: MAP_ID` (so the **light "Voyager/Light"** tile style is configured on the **cloud Map ID** — a `mapId` ignores inline `styles`, so the light grey-beige basemap lives on `NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID` in the Google Cloud console, NOT in code). Costa Rica focus: default center = **GAM** at **zoom 11** (single result → 13; multiple → `fitBounds` capped at 13), **`minZoom: 8`**, **`restriction` to CR bounds** (`strictBounds:false`) so you can't pan/zoom off the country. **Only the zoom control** (no fullscreen/streetview/maptype/rotate/scale); **`gestureHandling: "greedy"`** (Sprint 147) so the **mouse wheel zooms DIRECTLY** when the cursor is over the map — no "usa Ctrl + scroll" overlay — and mobile keeps one-finger pan + pinch-zoom. (Was `"cooperative"`; the Yelp mobile shell uses a fixed-viewport non-scrolling page, so greedy can't trap page scroll.) (Replaced the older raster + inline-`styles` + legacy-`Marker` basemap.)
...
- **Click / hover a pin → popup mini-card**: a second `AdvancedMarkerElement` floated above the pin — a white **`rounded-[14px]`** card (~**240px**, soft shadow, Inter) with an **initials avatar (light-blue `#EBF5FB`/`#009FD9`)**, name (`#111827`) + the **"Verificado" pill** when verified, profession labels, ★ rating (N), and the **"₡… /unit" price in blue `#008ce0`**; the whole card links to the profile, with a close ×. Closes on map click / another pin / ×. **Desktop hover** also opens this card as a preview (150ms close-debounce so pin→card / pin→pin doesn't flicker); mobile = tap.
  - **The popup "Verificado" matches the PROFILE badge** (Sprint 145): the SAME treatment as `Badge variant="verified"` — a **solid brand-blue `#009FD9` rounded-full pill, white text** (`.ccr-ver`), **never** green text + a `✓` check. Keep these in sync with the profile/card badge.
...
- **Clusters are INTERACTIVE, never dead** (Sprint 145): hover (desktop) / tap (mobile) on a cluster opens a **members-preview popup** that combines BOTH affordances — (1) a **scrollable list** of the grouped pros (each row = avatar + name + Verificado pill + ★rating + price, linking to that profile → *see & pick*), capped at 6 with a "+N más" line, AND (2) an explicit **"Acercar para separar / Zoom in to separate"** button that zooms the map into the cluster's member bounds (`zoomToCluster`, +2 fallback, clamp ≤17) → *separate*. **ONE safe gesture everywhere** (tap/click and hover both just open the preview); **zoom is an explicit button, not a competing gesture**, so there's no desktop-vs-mobile conflict. The MarkerClusterer's default **click-to-zoom is disabled** (`onClusterClick: () => {}`) so it can't fight the popup. Each individual marker carries `_pro` so the renderer can list its cluster's members (deduped by `proId`).
- **"Buscar en esta área" (search-this-area, Airbnb/Uber-style):** after a USER pan/zoom (suppressed during the programmatic `fitToMarkers`), a floating white pill button shows **top-center over the map**; clicking it reads `map.getBounds()` and `router.push`es `/buscar?...&n&s&e&w` (page reset). The server reads N/S/E/W into `searchProfessionals({ bounds })`, a **JS post-filter** keeping pros whose position (exact pin / a workplace / province centroid) is inside the viewport — **ADDED to** the active filters (category/sort/verified/…), not replacing them. The button hides again once results for the area load (`renderMarkers` resets it). Works desktop + mobile.
...
- **Master–detail layout (current).** LEFT = the **profession list** — each a selectable card showing the name, a **quiet blue "Principal" TEXT label** (no star, no pill) on the main one, and its **service count**; a dashed **"+ Agregar profesión"** sits at the bottom and opens a **searchable picker modal** (taxonomy via `ALL_CATEGORIES`, accent-insensitive search, **excluding already-added** professions). RIGHT = the **selected profession's services panel** — header (profession name + a **"Principal" pill** when it's the main one, else a **"Hacer principal"** action, + a remove trash when >1) with the **"N servicios publicados"** count, then the service rows (each a single bordered card: name, description, price, edit/delete), and a **"+ Agregar servicio"** at the bottom. Add/edit service is a **focused `Modal`** (name, short description, **price = ₡ amount + a unit `<select>` (/hora, /servicio…) + a "Precio a consultar (no mostrar monto)" checkbox**). All data/persistence is unchanged — autosave (`persist`) on every add/edit/remove/make-principal; `years` is preserved silently (no input in the new modal). The dashboard content heading for this tab is **"Profesiones y servicios"** (+ subtitle) while the sidebar nav stays **"Profesiones"**. (Replaced the older one-card-per-profession stacked layout.)
...
**Disponibilidad keeps labels icon-free.** No decorative icon next to "Disponibilidad privada", "Mis horarios de siempre", or "¿Un día distinto?" — the card headings + subtitles carry the structure; the toggles speak for themselves. The only icons in the editor are quiet grey utility marks (the "HORARIO PARA" calendar eyebrow, the `MapPin` on location tabs, the `Calendar` on the "Cambiar un día" action) — consistent with the serious-icon tone.
