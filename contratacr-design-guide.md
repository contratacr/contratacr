...
- **Password show/hide eye:** the icon shows the FUTURE-state affordance, matching the field state — **HIDDEN (dots) → `EyeOff` (slashed eye, "click to reveal")**, **VISIBLE (plain text) → `Eye` (open eye, "currently visible")**. So the ternary is `show ? <Eye/> : <EyeOff/>` (NOT the reverse). Same everywhere it appears: registration (cliente, profesional, booking modal), login, reset-password, change-password (account-security). **Each password field renders exactly ONE eye — our custom toggle, ALWAYS visible (from the start, even when the field is empty), never gated behind typing.** The shared **`Input` component auto-renders the eye** for any `type="password"` field when the caller doesn't pass its own `rightIcon` (it manages an internal reveal state) — so a plain `<Input type="password" />` (e.g. `/login`, admin login) gets the always-visible eye for free; fields that pass their own eye button keep it. **Never leave a password field without an eye** (a hardcoded `type="password"` with no toggle = no eye). The browser's NATIVE reveal control (Edge/IE `::-ms-reveal`/`::-ms-clear`) is globally hidden in `globals.css` (`display:none`); otherwise Windows browsers add a SECOND eye that appears only WHILE typing and toggles the field independently — looking like "two eyes" or an eye that "only shows after typing." If you ever see a duplicate / typing-gated / inverted eye, it's the native control resurfacing — keep that CSS rule.
