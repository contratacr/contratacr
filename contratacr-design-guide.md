...
- **Clean, low-border layout** (Calendly/Cal.com-style): **no numbered "STEP 1/STEP 2" badges**. Two bordered cards with plain headings — "Cómo te contactan los clientes" (privada + llamada toggles, divider-separated) and "Agrega tus horarios" (the generator). Upcoming slots render as **tidy per-day cards** (soft `bg-[#f9fafb]` header bar with the date + "Quitar día", body with optional profession/location tags) and **uniform same-width time chips** in an even grid (no Mañana/Tarde/Noche sub-labels — removed for simplicity).
