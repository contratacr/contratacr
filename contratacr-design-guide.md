...
**ONE account, two capabilities, ONE panel with a MODE SWITCH (Airbnb model — sprint 376).** Every account uses the SAME unified panel (physically still at `/dashboard/profesional`; `/dashboard/cliente` is now a thin redirector into it). At the top sits a **mode switch** — **"Modo cliente" ↔ "Modo profesional"** (en "Client mode" / "Professional mode") — that flips the whole panel between the user's two sections. **Labels = "Modo {cliente|profesional}" (sprint 383):** the word **"Modo"** frames each as a *switchable state*, not a fixed "what I am" (the earlier first-person "Busco/Ofrezco servicios" read like the account type they signed up as, so a pro didn't realize the client section was also theirs); "cliente/profesional" names the two facets most unambiguously. **Using "profesional" here is fine** — the anti-"profesional" rule is about not scaring people off BEFORE signup; inside the panel the user is already in. **The switch UI is built to read as "two sections you own and can move between," not a locked identity:** a one-line hint above it — **"Una cuenta, dos modos. Cambia cuando quieras."** with an `ArrowLeftRight` icon — then a segmented toggle where **BOTH options are always visible** (the inactive one is plainly "the other section you can switch to"), each with its **own icon** (cliente = `User`, profesional = `Briefcase`), the active one **filled white** + brand text, the inactive muted with a hover fill. Shown to EVERYONE (a seeker tapping "Modo profesional" lands on the activation gate → discovers that side is theirs to unlock).
...
> **Minimize the word "profesional" (sprint 374).** Never use it for IDENTITY/ACTION/registration/navigation/panel — use action framing there ("Ofrezco servicios" / "Busco servicios" / "Ofrecer mis servicios" / "Mi panel" / "¿Ofreces servicios?"). For **search/nav** say **"servicios"** ("Buscar servicios"); for **trust/saved/profile** say **"perfil(es)"** ("perfiles verificados", "Guardar perfil", "este perfil"); for the **provider as a person in prose**, prefer **rephrasing to avoid the noun** ("con la persona", "quienes ofrecen ese servicio", "te envían propuestas", "{n} resultados") or the actual trade. **Keep "cliente"** for the seeker as the provider sees them. EN: identity→action, person→trade/"the person"/"provider"/"profile"/rephrase, fall back to "pro"/"professional" only if unavoidable. The word survives ONLY where every alternative reads worse or it's structural: the **category group "Servicios profesionales"** (a service type), the **Terms of Service** legal term **"Profesional / Proveedor"**, and **quality adjectives** ("puntual y profesional"). Internal ids/routes/roles (`role==="professional"`, `/registro/profesional`) are unaffected.
>
