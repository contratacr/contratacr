...
ALL support is **tickets** — the ONLY channel. **No public email address, no WhatsApp, no live-chat** on `/soporte`, `/contacto`, `/ayuda`, or confirmations (the product↔professional WhatsApp coordination is separate and stays). Model: `support_tickets` + a **`support_ticket_messages` thread** (`sender_role` user|admin; distinct name — a legacy `support_messages` from migration 012 exists). Service-role only (RLS on, no policies).
- **Three statuses only:** **Pendiente** (`open`) → **En proceso** (`in_progress`) → **Resuelto** (`resolved`). NO "Cerrado". Admin tab label is **"Pendientes"** (not "Abiertos").
...
- **Confirmation** ("¡Ticket creado!"): honest response-time copy, no email/WhatsApp; logged-in → "Ver mis tickets" button, guest → email-reply message.
