...
- **"A domicilio" (traveling) availability is OPTIONAL and reuses the per-location schedule (Sprint 168).** A pro who enabled **"Me desplazo donde el cliente"** (`service_type` includes `"mobile"`) gets ONE extra location option in the schedule generator's location dropdown: **"A domicilio"** (`AvailabilityEditor` constant `A_DOMICILIO_LOC = "cov_domicilio"` — starts with `cov_` so it flows through the existing coverage-zone path), listed after any physical workplaces. They add time slots to it exactly like a physical location. It's **OPTIONAL** — add hours and "A domicilio" becomes a bookable tab on /buscar; add none and the pro keeps the WhatsApp-coordination behavior (§34). A hint under the dropdown (`aDomicilioHint`) states this. On /buscar, `cov_domicilio` already renders as an **"A domicilio"** tab (label via `locLabel`, no street address) with its own slots — NO /buscar changes were needed. The editor's empty-state `needLocation` copy points pros to "Me desplazo donde el cliente" (no longer to the removed videoconsulta).
