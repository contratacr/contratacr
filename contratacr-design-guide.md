...
- **PIN interaction — clean Airbnb/Google-Maps model** (rewritten Sprint 165; the canonical spec — don't patch around it):
  ...
  - **NO hover vibration — the ROOT cause + the DEFINITIVE fix** (Sprint 165, after 148/155/160 all failed by targeting the wrong element). The popup is a SEPARATE `AdvancedMarkerElement`; Google's `<gmp-advanced-marker>` wrapper has a transparent hit-box at the popup's UN-transformed layout position — directly **ON TOP OF the pin** (the card is only shifted up *visually* by the CSS transform). Default `pointer-events:auto` → it STEALS the pin's hover → mouseleave → close → mouseenter → flicker loop. **FIX = `neutralizePopup(marker, wrap)`:** `AdvancedMarkerElement` extends `HTMLElement`, so set **`pointer-events:none` directly on the marker element** (structure-independent — past DOM-walks missed deeper nesting), PLUS a wrapper-walk fallback (stops at `<gmp-advanced-marker>`, never the shared pane). The **card/rows keep `pointer-events:auto`** (a child's `auto` overrides the parent's `none`) so they stay clickable; the cursor passes straight THROUGH the popup to the pin. Position computed once on open; deduped by key. Applied to BOTH popups.
