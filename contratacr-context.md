...
1. **Desde/Hasta time picker — redesigned + validated.** New shared **`TimeSelect`** (`components/ui/time-select.tsx`): a polished 12-hour dropdown where every option is a full unambiguous label ("10:00 a.m.", "3:00 p.m.") — the meridiem is **intrinsic**, so selecting across noon flips a.m./p.m. automatically (no separate, sticky AM/PM control). Used for **Desde**, **Hasta** and **hora puntual** in the `AvailabilityEditor` (30-min grid, `min` = next full hour today). **Validation, no silent no-op:** `generate()` now blocks `end ≤ start` with an explicit message ("La hora de fin debe ser posterior a la de inicio."), shown inline on the "Hasta" field live AND surfaced on click; the existing CR past-date + 15-min-lead checks still fire with their messages. `to12h()` helper formats stored 24h values.
