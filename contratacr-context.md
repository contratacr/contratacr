...
- **`/buscar`** (the priority screen) — confirmed structurally on-spec (full-width `max-w-[1920px]` 3-column shell, numbered cards ↔ map pins, empty state, pagination). Fixed all EN leaks: pagination (`search.pagination.*`, `pageOf` via `t.rich`), `SearchFilters` (placeholder, "Cercanía", "Aseguradora", "Cualquiera", near-me button + geo errors, "Solo identidad verificada", clear-search/remove-insurer aria-labels → `search.filters.*` / `search.sort.cercania`), and `ProfessionalCard` coverage labels (`card.coverageCountry`/`coverageYourLocation`/`coverageAreas`, helper now takes the `card` translator). Fixed a voseo leak (`search.noResults.desc` "expandí"→"amplía"). **When auditing a public page, grep the component tree for bare Spanish string literals + voseo before assuming it's done** — several "translated" components still had hardcoded labels in non-obvious spots (aria-labels, select placeholders, geo errors, module-level helper functions).
