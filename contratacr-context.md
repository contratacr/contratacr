...
- **Method that works:** `grep -noE '"[A-ZÁÉÍÓÚÑ]...' ` the component tree for bare Spanish literals + voseo before assuming a "translated" page is done — leaks hide in aria-labels, select placeholders, geo/error strings, and module-level helper functions/const maps.
