...
export const PRICING_TYPES: { value: PricingType; label: string; suffix: string }[] = [
  ...
  { value: "a_convenir", label: "Precio a consultar", suffix: "" },
];
...
/** Human-readable single tier, e.g. "₡15,000/hora" or "Precio a consultar". */
export function formatPricingTier(tier: PricingTier): string {
  if (tier.type === "a_convenir") return "Precio a consultar";
  ...
  return tier.amount != null ? `${formatColones(tier.amount)}${suffix}` : "Precio a consultar";
}
 ...
 * priced service; if services exist but none are priced → "a consultar". Falls
...
export function deriveDisplayPricing(
  ...
  // Has services but none priced → show "a consultar".
