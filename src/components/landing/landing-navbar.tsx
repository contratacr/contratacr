...
function CategoriesMegaPanel({ onNavigate }: { onNavigate: () => void }) {
  ...
  return (
    <div className="flex max-h-[calc(100vh-7rem)] min-h-0 flex-col overflow-hidden">
      ...
      <div className="mb-4 flex h-11 shrink-0 items-center rounded-xl border border-gray-200 bg-gray-50/70 px-3.5 transition-all focus-within:bg-white focus-within:ring-2 focus-within:ring-[#009FD9]/20">
...
export function LandingNavbar({ mobileInline, forceCompactSearch = false, mobileSearch = false, marketplaceDesktop = false, drawerOnly = false }: { mobileInline?: React.ReactNode; forceCompactSearch?: boolean; mobileSearch?: boolean; marketplaceDesktop?: boolean; drawerOnly?: boolean } = {}) {
  ...
  // Los oficios del buscador vacío: uno por grupo y solo donde hay gente
  // registrada. Se eligen en el servidor con la cuenta de oferta (ver
  // `lib/data/oficios-de-arranque.ts`) y llegan aquí por el registro; el filtro
  // contra el catálogo vivo quita cualquiera que el administrador haya
  // escondido. Sin `useMemo` a propósito: son ocho comparaciones, y el catálogo
  // de la base llega después del primer pintado.
  const serviciosMasBuscados = (() => {
    const vivos = new Set(getAllCategories().map((c) => c.id));
    return oficiosDeArranque().filter((id) => vivos.has(id)).slice(0, 8);
  })();
          ...
          <div className="px-4 sm:px-6 lg:px-8">
            <div className={cn(
              ...
              {segundaLineaBuscador && (
                ...
                >
                  <div className="flex h-12 w-full items-center gap-3 rounded-[10px] border border-[#e5e7eb] bg-white px-3 transition-colors focus-within:ring-2 focus-within:ring-[#009FD9]/20">
                    <button
                      type="button"
