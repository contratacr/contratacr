  ...
  function BookingCard({ booking }: { booking: Booking }) {
    ...
    return (
      <Card id={`booking-${booking.id}`} className={cn("overflow-visible rounded-2xl border-[#e5e7eb] bg-white shadow-sm transition-[box-shadow,border-color] hover:shadow-md", expanded && "shadow-md ring-1 ring-[#d8eef8]")}>
        ...
        {expanded && (
          <div className="rounded-b-2xl border-t border-[#eef2f6] bg-gradient-to-b from-[#fcfdff] to-white px-4 pb-5 pt-4 sm:px-5 flex flex-col gap-4">
            {requestedDate && (
              <p className="flex items-center gap-1.5 text-[11px] text-[#68778d]">
            ...
            {location && (
              <div className="flex items-start gap-2.5">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#68778d]" />
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.06em] text-[#68778d]">{t("fieldZone")}</p>
            ...
            {booking.for_someone_else && (() => {
              ...
              return (
                <div className="flex items-start gap-2.5">
                  <UserRound className="mt-0.5 h-4 w-4 shrink-0 text-[#68778d]" />
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.06em] text-[#68778d]">{t("apptForLabel")}</p>
                    <div className="mt-0.5 flex flex-wrap items-center gap-2">
                      <p className="min-w-0 text-[13px] font-semibold text-[#162543] [overflow-wrap:anywhere]">
          ...
          {actionFor?.id === booking.id && actionFor.mode === "cancel" && (
            <div className="rounded-xl border border-[#e5e7eb] bg-[#fafafa] p-3 flex flex-col gap-2.5">
              ...
              <textarea
                ...
                rows={2}
                maxLength={300}
                className="w-full rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm text-[#162543] placeholder:text-[#68778d] break-words focus:outline-none focus:ring-2 focus:ring-[#009FD9] focus:border-transparent resize-none"
              />
