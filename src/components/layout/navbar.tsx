...
function LanguageToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className="px-2.5 py-1 rounded-lg border border-[#e5e7eb] text-xs font-semibold text-[#374151] hover:border-[#319278] hover:text-[#319278] transition-colors"
    ...
    <header className="sticky top-0 z-50 w-full border-b border-[#e5e7eb] bg-white/95 backdrop-blur-sm">
      ...
      <div className={cn("md:hidden border-t border-[#e5e7eb] bg-white overflow-hidden transition-all duration-200", open ? "max-h-80" : "max-h-0")}>
        <nav className="px-4 py-4 flex flex-col gap-1">
          ...
          <Link href="/categorias" className="px-3 py-2.5 rounded-xl text-sm font-medium text-[#374151] hover:bg-[#f0f9f6] hover:text-[#319278] transition-colors" onClick={() => setOpen(false)}>
            {t("categories")}
          </Link>
          <Link href="/como-funciona" className="px-3 py-2.5 rounded-xl text-sm font-medium text-[#374151] hover:bg-[#f0f9f6] hover:text-[#319278] transition-colors" onClick={() => setOpen(false)}>
            {t("howItWorks")}
          </Link>
