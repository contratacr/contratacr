    ...
    <>
      ...
      <div className="relative w-full h-full" onMouseMove={handleHoverProximity} onMouseLeave={hidePopup}>
        ...
        <button
          type="button"
          onClick={searchThisArea}
          className="fixed left-[4.25rem] right-[7.5rem] top-[calc(env(safe-area-inset-top)+5rem)] z-40 inline-flex h-10 min-w-0 translate-x-0 items-center justify-center gap-1.5 overflow-hidden whitespace-nowrap rounded-full border border-[#e5e7eb] bg-white px-3 text-[13px] font-semibold text-[#162543] shadow-[0_8px_24px_rgba(15,23,42,0.16)] transition hover:bg-[#f9fafb] active:scale-95 min-[390px]:left-[7rem] min-[480px]:px-4 min-[480px]:text-sm lg:hidden"
        >
          <Search className="h-4 w-4 shrink-0 text-[#008ce0]" />
          <span className="min-w-0 truncate">{locale === "en" ? "Search this area" : "Buscar en esta área"}</span>
        </button>
        <div className="fixed right-3 top-[calc(env(safe-area-inset-top)+5rem)] z-40 flex gap-2 lg:hidden">
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full border border-[#e5e7eb] bg-white text-2xl font-light leading-none text-[#374151] shadow-[0_8px_24px_rgba(15,23,42,0.16)] transition hover:bg-[#f9fafb] active:scale-95"
          >
            +
          </button>
          <button
